...
export class SearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.chargingStationService.getChargingStations().subscribe(chargingStations => {
  ...
  filterChargingStation(searchTerm: string): ChargingStation[] {
    ...
    return this.allChargingStation.filter(station =>
      station.location.streetName.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }

  selectStation(station: ChargingStation): void {
    this.searchControl.setValue(station.location.streetName, { emitEvent: false });
